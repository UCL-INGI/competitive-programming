...
def check_cycle(graph, cycle):
  if cycle[0] != cycle[-1]:
    return False, 'the cycle should start and end at the same node'
  edge_set = set()
  n = len(cycle)
  for i in range(1, n):
    e = (cycle[i - 1], cycle[i])
    if e in edge_set:
      return False, 'the cycle contains repeated edges'
    if not e in graph.edge_set:
      return False, 'the cycle contains edges that are not in the graph'
    edge_set.add(e)
  node_set = set()
  for i in range(0, n - 1):
    u = cycle[i]
    if u in node_set:
      return False, 'the cycle contains repreated nodes'
  return True, ''
